...
class DSPyEducationalSystem:
    ...
    def __init__(self, use_type_aware_pipeline: bool = True):
        self.prompt_builder = ModularPromptBuilder()
        self.main_lm = None
        self.expert_lms = None
        self.pipeline = None
        self.initialized = False
        self.use_type_aware_pipeline = use_type_aware_pipeline
    ...
    async def initialize(self):
        ...
        try:
            ...
            # Create educational pipeline (type-aware or traditional)
            if self.use_type_aware_pipeline:
                logger.info("Using TypeAwareEducationalPipeline with refinement decision engine")
                self.pipeline = TypeAwareEducationalPipeline(
                    expert_lms=self.expert_lms,
                    prompt_builder=self.prompt_builder
                )
            else:
                logger.info("Using traditional GermanEducationalPipeline")
    ...
    async def generate_questions_dspy(self, request: SysArchRequest) -> tuple[SysArchResult, Dict[str, Any]]:
        ...
        try:
            ...
            # Extract validated questions (handle both typed and traditional results)
            validated_questions = pipeline_result['validated_questions']
            typed_questions = pipeline_result.get('typed_questions', [])
            ...
            if 'pipeline_details' in pipeline_result:
                if 'typed_generation' in pipeline_result['pipeline_details']:
                    # Type-aware pipeline
                    initial_generation = pipeline_result['pipeline_details']['typed_generation']['generation_result']
                    initial_questions = initial_generation.get('questions', [])
                elif 'initial_generation' in pipeline_result['pipeline_details']:
                    # Traditional pipeline
                    initial_questions = pipeline_result['pipeline_details']['initial_generation'].get('questions', [])
            
            # Build SYSARCH-compliant CSV data with initial questions and typed data
            csv_data = self._build_sysarch_csv(validated_questions, request_params, initial_questions, typed_questions)
            ...
            generation_updates = [
                {
                    ...
                    "expert_consensus": [q['approved'] for q in validated_questions],
                    "dspy_metadata": {
    ...
    def _build_sysarch_csv(self, validated_questions: List[Dict], request_params: Dict[str, Any], initial_questions: List[str] = None, typed_questions: List = None) -> Dict[str, Any]:
        ...
        csv_data = {
            ...
            # Type-aware pipeline metadata (if available)
            "typed_pipeline_used": bool(typed_questions),
            "refinement_applied": [q.refinement_applied for q in typed_questions] if typed_questions else [],
            "format_preserved": [q.format_preserved for q in typed_questions] if typed_questions else [],
            "expert_average_ratings": [q.get_average_rating() for q in typed_questions] if typed_questions else [],
...
# FastAPI app integration with lifespan events
...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        logger.info("DSPy orchestrator shutdown")

app = FastAPI(title="DSPy Educational Question Generator", lifespan=lifespan)
    ...
    try:
        ...
        try:
            ...
            if 'pipeline_details' in pipeline_result:
                ...
                # Save initial generation step (handle both typed and traditional pipelines)
                if 'typed_generation' in details:
                    # Type-aware pipeline
                    typed_gen = details['typed_generation']
                    result_manager.save_dspy_pipeline_step(
                        step_name="01_typed_generation",
                        step_data={
                            "typed_questions": [q.model_dump() for q in typed_gen.get('typed_questions', [])],
                            "question_types_generated": typed_gen.get('question_types_generated', []),
                            "generation_result": typed_gen.get('generation_result', {}),
                            "model_used": "llama3.1:8b (port 8001)",
                            "typed_models_used": True
                        },
                        step_metadata={
                            "step_description": "Type-aware question generation with Pydantic models",
                            "processing_time_ms": typed_gen.get('processing_time_ms', 0),
                            "model_port": 8001,
                            "questions_generated": len(typed_gen.get('typed_questions', [])),
                            "pipeline_type": "type_aware"
                        }
                    )
                elif 'initial_generation' in details:
                    # Traditional pipeline
                    result_manager.save_dspy_pipeline_step(
                ...
                # Save consensus or refinement step results 
                if 'refinement_decisions' in details:
                    # Type-aware pipeline refinement decisions
                    refinement_data = details['refinement_decisions']
                    result_manager.save_dspy_pipeline_step(
                        step_name="03_refinement_decisions",
                        step_data={
                            "refinement_summary": refinement_data.get('summary', {}),
                            "questions_refined": refinement_data.get('questions_refined', 0),
                            "refinement_rate": refinement_data.get('refinement_rate', 0.0),
                            "format_preservation_rate": refinement_data.get('format_preservation_rate', 0.0),
                            "refinement_algorithm": "expert_consensus_with_decision_engine"
                        },
                        step_metadata={
                            "step_description": "Type-aware refinement decisions with format preservation",
                            "processing_time_ms": refinement_data.get('processing_time_ms', 0),
                            "questions_processed": len(pipeline_result.get('typed_questions', [])),
                            "pipeline_type": "type_aware"
                        }
                    )
                elif 'consensus' in details and 'results' in details['consensus']:
                    # Traditional pipeline consensus
            ...
            result_manager.save_final_results(
                ...
                final_metadata={
                    ...
                    "type_aware_pipeline": dspy_system.use_type_aware_pipeline,
                    "single_pass_consensus": True,
                    "expert_consensus_achieved": result.csv_data.get('dspy_all_approved', False),
                    "typed_questions_used": bool(pipeline_result.get('typed_questions', [])),
                    "refinement_decisions_made": 'refinement_decisions' in pipeline_result.get('pipeline_details', {}),
